...
COVER_IMAGE_LIMIT = 1024 * 1024
COVER_MIN_WIDTH = 900  # 微信封面建议宽度下限，过小会触发 53401
ALLOWED_IMAGE_MIME = {"image/jpeg", "image/png"}
DEFAULT_CREDENTIALS_FILE = Path.home() / ".config" / "wechat-mp" / "wechat.env.profile"
...
def _load_pillow():
    """按需导入 Pillow；缺失时返回 None，让调用方回退到只校验+给提示。"""
    try:
...
def compress_image_for_wechat(path: Path, limit: int, min_width: int = 0) -> Path | None:
    """把超限图片压到 limit 以下，输出到同目录的 *.wechat.jpg（不覆盖原图）。

    需要 Pillow；不可用时返回 None，由调用方给出可执行提示。
    """
    Image = _load_pillow()
    if Image is None:
        return None
    out = path.with_suffix(".wechat.jpg")
    try:
        img = Image.open(path).convert("RGB")
        # 尺寸过小会触发封面 53401，先按需放大到建议宽度
        if min_width and img.width < min_width:
            new_h = int(img.height * min_width / img.width)
        ...
        while quality >= 55:
            ...
            if out.stat().st_size < limit:
                return out
            quality -= 5
        # 质量降到底仍超限则逐步缩小尺寸（不低于建议宽度）
        while out.stat().st_size >= limit and img.width > max(min_width, 320):
...
def check_inline_image(path: Path) -> Path:
    """校验文内图，必要时压缩。返回最终应上传的路径（可能是压缩副本）。"""
    ...
    if path.stat().st_size <= INLINE_IMAGE_LIMIT:
        return path
    compressed = compress_image_for_wechat(path, INLINE_IMAGE_LIMIT)
    if compressed is not None:
        print(
            f"note: inline image over 1 MB, using compressed copy {compressed.name}",
            file=sys.stderr,
        )
        return compressed
    raise WeChatError(
        f"WeChat article images must be under 1 MB: {path} "
        f"({path.stat().st_size / 1024 / 1024:.2f} MB). "
        "Install Pillow (pip install Pillow) for auto-compression, "
        "or convert to JPEG / shrink the file manually."
    )


def check_cover_image(path: Path) -> Path:
    """校验封面（格式/大小/最小宽度），必要时压缩。返回最终应上传的路径。"""
    require_file(path, "Cover image")
    mime = mimetypes.guess_type(path.name)[0]
    if mime not in ALLOWED_IMAGE_MIME:
        raise WeChatError(
            f"WeChat cover must be jpg/png: {path}. "
            "Convert PNG/other formats to JPEG before uploading."
        )
    Image = _load_pillow()
    too_big = path.stat().st_size > COVER_IMAGE_LIMIT
    too_small = False
    if Image is not None:
        try:
            with Image.open(path) as im:
                too_small = im.width < COVER_MIN_WIDTH
        except Exception:
            pass
    if not too_big and not too_small:
        return path
    compressed = compress_image_for_wechat(path, COVER_IMAGE_LIMIT, min_width=COVER_MIN_WIDTH)
    if compressed is not None:
        print(
            f"note: cover adjusted for WeChat (size/width), using {compressed.name}",
            file=sys.stderr,
        )
        return compressed
    reason = "over 1 MB" if too_big else f"narrower than {COVER_MIN_WIDTH}px"
    raise WeChatError(
        f"WeChat cover invalid ({reason}), likely error 53401: {path}. "
        "Install Pillow (pip install Pillow) for auto-fix, "
        f"or provide a JPEG under 1 MB and at least {COVER_MIN_WIDTH}px wide."
    )
...
def get_access_token(args: argparse.Namespace) -> str:
    ...
    if credentials_file:
        file_credentials = load_credentials_file(Path(credentials_file).expanduser())
    else:
...
def rewrite_inline_images(content: str, base_dir: Path, token: str | None, dry_run: bool, upload: bool) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if path is None:
            ...
            return match.group(0)
        if dry_run:
            # dry-run 也校验图片规格，把格式/大小问题在发布前暴露出来
            final = check_inline_image(path)
            new_url = f"WECHAT_UPLOADED_IMAGE_URL:{final.name}"
        else:
            if token is None:
...
def command_draft(args: argparse.Namespace) -> None:
    ...
    if not thumb_media_id:
        if args.dry_run:
            # dry-run 不上传，但仍预检封面规格，避免真实运行才发现 53401
            if cover:
                check_cover_image(Path(cover).expanduser().resolve())
...
def add_auth_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--credentials-file",
        help="Path to a credentials profile file (default: ~/.config/wechat-mp/wechat.env.profile).",
    )
