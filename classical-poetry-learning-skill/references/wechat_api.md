...
| 封面 thumb | jpg / png | < 1 MB | 宽度建议 ≥ 900px，过小触发 `53401` |

- 配图阶段就应产出合规文件：文内图控制在 1MB 以内，封面用 JPEG、宽度 ≥ 900px 且 < 1MB。PNG 常因体积大而超限，封面优先导出 JPEG。
- `wechat_mp_publish.py` 会在 **dry-run 阶段就校验**文内图和封面；装有 Pillow 时自动生成压缩副本 `*.wechat.jpg`（不覆盖原图），否则报错并给出手动修复提示。
- 正文 HTML（`content` 字段）本身也不能超过 1 MB。
...
- `48001`：账号未获得该 API 权限。
- `40005`：上传图片文件类型无效（必须 jpg/png）。
- `40009`：图片体积过大（文内图/封面需 < 1 MB）。
- `53401`：封面图片尺寸不合法（多为过大或宽度过小，改用 JPEG、宽度 ≥ 900px 且 < 1MB）。
- `53503`：草稿未通过发布检查。
