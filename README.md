...
Claude Code 技能集合，覆盖企业开发全流程、数据库运维、API 文档、文章排版与配图、内容平台发布、邮件管理、飞书协同等场景。
...
| [markdown-post-skill](./markdown-post-skill) | Markdown 发布到内容平台 | 走 Playwright 接入已登录 Chrome，发掘金 / 飞帆，本地图片自动上传 CDN |
| [article-illustration-tools](./article-illustration-tools) | 文章自动配图 | 分析文章结构、推荐图片位置、调用 Codex / Antigravity 生图并插入 |
| [image-factory-skill](./image-factory-skill) | AI 生图 + 多端分发 + Prompt 工程 | 一句描述生图，发飞书 / 发小红书 / 发抖音；反推 prompt、优化 prompt、生成人物写真 prompt |
...
### 6. markdown-post-skill — Markdown 发布到内容平台

把本地 Markdown 文章发布到内容平台（当前支持**稀土掘金** `juejin.cn`、讯飞内部的**飞帆** `feifan.iflytek.com`）。这些平台没有开放 API，只能走浏览器，本 skill 把两个核心难点一次解掉：

- **登录态复用**：走 Playwright `connect_over_cdp` 接入用户已登录的 Chrome，账号 / cookie / SSO 全部继承；连不上自动回落到 Playwright 自带 chromium + 持久化独立 profile
- **本地图片**：发布前把每张本地图片走平台自己的上传通道传到平台 CDN，再把链接替换回 markdown
- **frontmatter 预填**：自动填分类 / 标签 / 封面 / 摘要
- **默认存草稿**：脚本写完正文、传完图、填好元数据就停，把链接还给用户人工核对再发，避免误发
- 平台支持：macOS / Windows（Linux 暂未实现剪贴板分支）
...
依赖：`playwright>=1.40`，Python ≥ 3.9。

### 7. article-illustration-tools — 文章自动配图
...
### 8. image-factory-skill — AI 生图 + 多端分发 + Prompt 工程

一站式 AI 生图工具，三类核心能力：

**A. 生图 + 多端分发**
- **发飞书**：一句描述生成图片 → 推送到飞书的人或群；图片、配文与 prompt 合成**一条**富文本消息；多目标时图片只上传一次、并发发送
- **发小红书**：自动发布为图文笔记（标题 / 正文 / 话题 / 多图），走 Playwright 持久化浏览器会话
- **发抖音**：自动发布为抖音图文（标题 / 正文 / 话题 / 多图），走 Playwright 持久化浏览器会话

**B. Prompt 工程（用 codex-cli / agy）**
- **反推 prompt**：输入图片 → 推导出可复用的文生图 prompt
- **优化 prompt**：按风格预设（手绘 / 蓝图 / 水彩 / 赛博朋克 / 3D / 治愈 / 极简 / 写实 / 古风人物 / 写实人物）改写已有 prompt
- **生成人物写真 prompt**：一句主体描述 → 结构化人物写真 prompt（含风格 / 镜头 / 光线 / 画质 / 负向词）

**C. 通用**
- 后端：codex-cli 或 agy（Antigravity CLI），自动检测
- 飞书接收人通过 `.env` 配置 `FEISHU_USER_IDS` / `FEISHU_CHAT_IDS`
- 依赖：`lark-cli`（飞书）、`playwright`（小红书 / 抖音）

```
生成一张科技感的系统架构图，发给张三
给研发群发一张手绘风格的产品路线图，配文：Q2 规划草图
把这张图反推成 prompt
给我写一条古风人物写真 prompt：撑伞少女，江南雨巷
生成一张治愈系小猫，发一篇小红书笔记
```

### 9. enterprise-email-manager — 企业邮件管理器
...
# 浏览器自动化（markdown-post-skill、image-factory-skill 的小红书/抖音）
...
| Markdown 发到掘金 / 飞帆 | markdown-post-skill |
| 给长文自动配插图 | article-illustration-tools |
| 临时一张图发飞书 / 小红书 / 抖音 | image-factory-skill |
| 反推 / 优化 / 生成人物写真 prompt | image-factory-skill |
